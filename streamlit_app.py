...
st.set_page_config(page_title="Rough Path Signature Kernel with Macro Conditioning", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📈 Rough Path Signature Kernel with Macro Conditioning</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Path signature features + Gaussian kernel + ridge regression | Macro‑conditioned prediction</p>', unsafe_allow_html=True)
...
st.sidebar.markdown(f"**Signature depth:** {config.SIG_DEPTH} | **Bandwidth σ:** {config.KERNEL_BANDWIDTH}")
st.sidebar.markdown(f"**Ridge α:** {config.ALPHA}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rough_path_kernel_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Predicted return: {etf['rk_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Prediction"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Prediction", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📈 Top ETFs by Rough Path Signature Kernel Prediction (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Rough path theory** generalises Itô calculus to paths of low regularity.
        - The **path signature** (Chen 1958) provides a universal set of features for a path.
        - We compute truncated signatures (depth 4) of rolling return windows.
        - A product Gaussian kernel on signature + macro variables is used.
        - Kernel ridge regression predicts next‑day return.
        - The score is the predicted return – higher = stronger long signal.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the predicted returns.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Rough Path Signature Kernel | Kernel ridge regression on path signatures with macro conditioning")
